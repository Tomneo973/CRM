...
const bsg=(s,t)=>{const m={devis:{brouillon:'bdr',envoye:'bi',accepte:'bo',refuse:'bk'},facture:{brouillon:'bdr',envoyee:'bi',payee:'bo',retard:'bk',partiel:'bw'}};const l={brouillon:'Brouillon',envoye:'Envoye',envoyee:'Envoyee',accepte:'Accepte',refuse:'Refuse',payee:'Payee',retard:'En retard',partiel:'Paiement partiel'};return'<span class="badge '+(m[t]?.[s]||'bdr')+'">'+(l[s]||s)+'</span>'};
...
/* === PAIEMENTS === */
const paiementsOf=it=>Array.isArray(it.paiements)?it.paiements:[];
const totalPaye=it=>paiementsOf(it).reduce((s,p)=>s+(parseFloat(p.montant)||0),0);
const soldeDu=it=>Math.max(0,(it.totalTTC||0)-totalPaye(it));
const statutPaiement=it=>{const p=totalPaye(it),t=it.totalTTC||0;if(p<=0)return null;if(p>=t)return'payee';return'partiel'};
