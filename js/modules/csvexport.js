...
function expFacturesCSV(start,end){
  ...
  const headers=[{label:'Numero',get:r=>r.numero},{label:'Date',get:r=>fd(r.date)},{label:'Echeance',get:r=>fd(r.dateEcheance)},{label:'Client',get:r=>{const c=cl.find(x=>x.id===r.clientId);return c?c.nom:''}},{label:'Statut',get:r=>r.statut},{label:'Total HT',get:r=>(r.totalHT||0).toFixed(2)},{label:'Taux TVA (%)',get:r=>tvxOf(r)},{label:'Total TVA',get:r=>(r.totalTVA||0).toFixed(2)},{label:'Total TTC',get:r=>(r.totalTTC||0).toFixed(2)},{label:'Devise',get:r=>r.devise||'CHF'},{label:'Paye',get:r=>totalPaye(r).toFixed(2)},{label:'Solde du',get:r=>soldeDu(r).toFixed(2)},{label:'Notes',get:r=>r.notes||''}];
...
function expClientsCSV(){
  const rows=gd(K.cl);
  if(!rows.length){toast('Aucun client a exporter','error');return}
  const headers=[{label:'Nom',get:r=>r.nom||''},{label:'Entreprise',get:r=>r.entreprise||''},{label:'Email',get:r=>r.email||''},{label:'Telephone',get:r=>r.tel||''},{label:'Adresse',get:r=>r.adresse||''},{label:'Rue',get:r=>r.rue||''},{label:'N°',get:r=>r.rueNo||''},{label:'NPA',get:r=>r.npa||''},{label:'Ville',get:r=>r.ville||''},{label:'Pays',get:r=>r.pays||''},{label:'Notes',get:r=>r.notes||''},{label:'Date creation',get:r=>fd(r.dateCreation)}];
  downloadCSV('clients.csv',toCSV(rows,headers));toast(rows.length+' client(s) exporte(s)','success');
}
